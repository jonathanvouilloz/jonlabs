# Application mobile ou PWA : comment choisir pour ta PME ?
...
**App de chantier — photos géolocalisées, rapports hors ligne.** L'accès simultané à la caméra, au GPS et au stockage local en mode déconnecté dépasse ce que la PWA gère proprement aujourd'hui. Une app native iOS/Android construit sur [Flutter](/blog/flutter-vs-react-native-pme) ou React Native est la bonne réponse.

**Programme de fidélité avec paiements in-app.** Dès que tu touches aux achats intégrés via les stores, une app native s'impose. Les mécaniques de paiement d'Apple et Google sont étroitement liées à leurs écosystèmes.
...
- Tu réponds **non / non / non** (budget > 20 000 CHF, pas de SEO, pas d'accès hardware) → les deux options sont viables. Pose-toi la question : est-ce que la présence sur les stores a une valeur marketing pour toi ?
...
Pas directement. Une PWA peut être encapsulée dans une coque native via des outils comme Capacitor ou TWA (Trusted Web Activity pour Android), ce qui permet une publication sur le Play Store notamment. Mais c'est un travail supplémentaire — et ça sort du modèle "simple PWA" à budget réduit.
...
- Pinterest Engineering — migration vers PWA et résultats d'engagement (+60%), cas d'usage référencé dans le brief
- Starbucks — déploiement PWA avec fonctionnement hors ligne pour commandes sans connexion, cas d'usage référencé dans le brief
...
- Topical map et brief SEO Jon Labs — cluster application mobile, données de prix et critères de décision
