...
```mermaid
graph TD
  HUB["🎯 HUB CONVERSION<br/>Consultant IA Genève<br/>/consultant-ia (À créer)"]
  INFO["📚 HUB INFO (existant)<br/>/blog/automatisation-pme-suisse-guide-complet"]
  HUB <-.maillage.-> INFO

  HUB --> A["⬤ CORE · Services IA"]
  HUB --> B["⬤ CORE · Géo consultant-ia-[ville]"]
  HUB --> C["⬤ CORE · Métiers verticaux"]
  HUB --> D["⬤ CORE · Produits Hermès/OpenClaw"]
  INFO --> E["○ OUTER · Fondamentaux (existant)"]
  INFO --> F["⬤ Agents & outils (existant)"]
  INFO --> G["⬤ Nouveaux articles (gaps)"]

  B --> B1["consultant-ia-geneve · P1"]
  C --> C1["ia-fiduciaire · P2"]
  C --> C2["ia-agence-immobiliere · P2"]
  D --> D1["hermes · P2"]
  G --> G1["agent-ia-entreprise-cas-usage · P1 ⚡"]
  G --> G2["prix-agent-ia-automatisation · P1 ⚡"]
  G1 -.cas d'usage.-> C
  G2 -.prix.-> HUB
  D1 -.source contenu.-> F
```
