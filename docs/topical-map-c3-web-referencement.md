...
graph TD
  BRAND["🏷️ JON LABS"]
  BRAND --> A["🎯 C3a — Création web<br/>/services/creation-site-web"]
  BRAND --> B["🎯 C3b — SEO local<br/>/services/referencement-local"]
  BRAND --> C["🎯 C3c — GEO/IA<br/>/blog/geo-seo-ia (hub)"]
  BRAND --> D["🎯 C3d — GMB<br/>/services/gestion-fiche-google"]

  A --> A1["prix-site-web 432imp 🟢"]
  A --> A2["specialiste-dev-web 410imp 🟢"]
  A --> A3["15-signes-site-dort ⚡"]
  A --> A4["refonte-site-web-pme<br/>À CRÉER"]
  A --> AG["5 pages géo ville<br/>0 lien → MAILLER"]

  B --> B0["PILIER 693imp pos41"]
  B0 -.301.-> B1["referencement-local-geneve"]
  B0 -.301.-> B2["tarif-package-seo-local"]
  B --> B3["referencement-local-lausanne 🗓️"]

  C --> C1["6 moteurs : perplexity/claude/<br/>copilot/grok/chatgpt/gemini"]
  C --> C2["⚠️ réécrire titles (0 clic)"]

  D --> DH["hub guide-complet"]
  DH --> D1["créer/optimiser (5)"]
  DH --> D2["avis & posts (4)"]
  DH --> D3["suspension (1)"]

  %% Ponts (Contextual Bridges) :
  A3 -.site invisible.-> C
  C -.visibilité locale.-> D
  B -.GBP = levier SEO local.-> D
```
